...
Session::Session(SessionType type, QWidget* parent, Session *previousSession) : QObject(parent)
{
    ...
    setupSession(type, previousSession);
}
...
void Session::setupSession(SessionType type, Session *previousSession)
{
    Terminal* previousTerminal = 0;
    if (previousSession != 0) {
        previousTerminal = previousSession->getTerminal(previousSession->activeTerminalId());
    }
    switch (type)
    {
        case Single:
        {
            Terminal* terminal = addTerminal(m_baseSplitter, previousTerminal);
        ...
        case TwoHorizontal:
        {
            ...
            Terminal* terminal = addTerminal(m_baseSplitter, previousTerminal);
            addTerminal(m_baseSplitter, previousTerminal);
        ...
        case Quad:
        {
            ...
            Terminal* terminal = addTerminal(upperSplitter, previousTerminal);
            addTerminal(upperSplitter, previousTerminal);

            addTerminal(lowerSplitter, previousTerminal);
            addTerminal(lowerSplitter, previousTerminal);
...
Terminal* Session::addTerminal(QWidget* parent, Terminal* previousTerminal)
{
    Terminal* terminal = 0;
    if (previousTerminal == 0) previousTerminal = getTerminal(activeTerminalId());

    if (previousTerminal == 0)
        terminal = new Terminal(parent);
    else
        terminal = new Terminal(parent, previousTerminal->getCwd());
